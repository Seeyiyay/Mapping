...
public abstract class GUI {
	...
	protected abstract void articulateIterStart();

	protected abstract void articulateRecurStart();
	/**
	...
	public JComponent drawing; // we customise this to make it a drawing pane.
	private JTextArea textOutputArea;

	private JTextField search;
	private JFileChooser fileChooser;
	private JCheckBox useSpeed;
	...
	private void initialise() {
			...
			public void actionPerformed(ActionEvent ev) {
				...
				if (fileChooser.showOpenDialog(frame) == JFileChooser.APPROVE_OPTION) {
					...
					for (File f : files) {
						...
						else if (f.getName().endsWith(REST_FILENAME)) {
							restrictions = f;
						}
					}
		...
		JButton articulateIterative = new JButton("ArtIter");
		articulateIterative.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent ev) {
				articulateIterStart();
				redraw();
			}
		...
		JButton articulateRecursive = new JButton("ArtRecur");
		articulateRecursive.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent ev) {
				articulateRecurStart();
				redraw();
			}
		});

		JCheckBox useSpeed = new JCheckBox("Use speed");
		...
		controls.add(articulateIterative);
		controls.add(articulateRecursive);
		controls.add(useSpeed);
		controls.add(Box.createRigidArea(new Dimension(15, 0)));
